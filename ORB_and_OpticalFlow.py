...
def ORB_OptFlow_algorithm(video):
    orb = cv2.ORB_create(NUMBER_FEATURES)
    ...
    keypoint, descriptor = orb.detectAndCompute(previousFrame, None)
    corners = []
    ...
    for t in range(MAX_FRAMES):
        ...
        keypoint, descriptor = orb.detectAndCompute(actualFrame, None)
        nextPoints, status, error = cv2.calcOpticalFlowPyrLK(previousFrame, actualFrame, corners, None, **lk_params)


        # If remaining tracked keypoints are less than TRACKED_POINTS => give new keypoints to tracking algorithm
        ...
        if(t > 0):
            ...
            cv2.imshow('ORB + Optical Flow', finalFrame)
